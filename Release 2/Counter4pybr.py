...
def raw_xml_rep( lg, mnth = 3, yr = 2019, report = "JR1", release = 4):
	...
	response = requests.post(url=wsdl_url, headers=headers, data=payload, verify=verify) # Post the SUSHI tree to the server and save the response as 'response'
...
def br1_df(lgn,month,year):
	...
	try:
		...
		print(etree.tostring( etree.fromstring(raw_xml_rep( lg = lgn, mnth = month, yr = year, report = 'BR1')), pretty_print = True, encoding = 'unicode')) 		#look at the whole tree
		rootp = repo[0][0][3][0][1]
