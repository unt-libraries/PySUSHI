...
class Counter5Report:
	...
	def __init__(self, year, month, report, url, requestor_id, customer_reference, platform_name = None, api_key = None, request_lowercase = '-', data_type = None, access_type = None):
		###=======================================================================================================
		...
		if report in report_set:
			...
			if report == "TR_J3":
				self.colset = TR_COLUMNS
			if report == "DR_D1":
				self.colset = DR_D1_COLUMNS
			if report == "TR_B3":
				self.colset = TR_COLUMNS
			if report == "TR":
				self.colset = TR_COLUMNS
			...
			if request_lowercase == 'x':
				report = report.lower()
			
			## If there are any filters activated in the parameters, populate the filter switches.
			if report == "TR":
				if (data_type != None) and (data_type.upper() == "BOOK"):
					self.data_type_filter = "Book"
				if (data_type != None) and (data_type.upper() == "JOURNAL"):
					self.data_type_filter = "Journal"
				if (access_type != None) and (access_type.upper() == "REGULAR"):
					self.access_type_filter = "Regular"
				if (access_type != None) and (access_type.upper() == "CONTROLLED"):
					self.access_type_filter = "Controlled"
				if (access_type != None) and (access_type.upper() == "OA_GOLD"):
					self.access_type_filter = "OA_Gold"
		else:
		...
		if requestor_id != '-' and requestor_id != None:
			req_params["requestor_id"] = requestor_id
			
		if customer_reference != '-' and customer_reference != None:
			...
			## If no exceptions are present, parse the report and complete.
				...
				try:
					if report.upper() == "TR_J3":
						self.data_type_filter = "Journal"
						self.trparse(report)
					if report.upper() == "DR_D1":
						self.drd1parse(report)
					if report.upper() == "TR":
						self.trparse(report)
					if report.upper() == "TR_B3":
						self.data_type_filter = "Book"
						self.trparse(report)
					if report.upper() == "TR_B2":
						self.data_type_filter = "Book"
						self.trparse(report)
					if report.upper() == "TR_J2":
						self.data_type_filter = "Journal"
						self.trparse(report)
				except:
